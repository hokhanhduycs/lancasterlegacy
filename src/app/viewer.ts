...
import { Frustum, Scene, WebGLRenderer, PerspectiveCamera, Vector2 } from 'three';
...
export default class PanoViewer {
    ...
    async loadPano(panodata: any, firstload?: boolean,anim=false) {
        const guid = panodata.guid
        // const hp =new Hotspot()
        // hp.pos = this.pano.pos
        // this.transition.moveTo(this.pano,hp)
        if(!this.transition.isRunning){
            console.log(this.pano)
            const hp = new Hotspot()
            hp.pos = this.pano.pos
            hp.type = 2
            this.transition.moveTo(this.pano,hp,0)
        }
