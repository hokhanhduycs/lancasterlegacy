import { Euler, Group, Quaternion, Matrix4, PlaneGeometry, MeshBasicMaterial, Mesh, Frustum, PerspectiveCamera, Texture, TextureLoader, Object3D,  ImmediateRenderObject, SphereGeometry, DoubleSide, Vector3, Vector2, NearestFilter, LinearMipmapNearestFilter, LinearFilter, NearestMipMapLinearFilter } from 'three';
...
export default class Pano {
    ...
    cleanObj(obj:Object3D){
        obj.traverse(x => {
            const obj = x as ImmediateRenderObject
            if (obj.material) {
                this.cleanMaterial(obj.material as MeshBasicMaterial)
            }
        })
    }
    clear() {
    ...
    loadPano(data:any) {
        ...
        this.obj.traverse(tile=>{
            const obj = tile as ImmediateRenderObject
            if(obj.material){
    ...
    async loadHighTexture(obj: Object3D) {
        ...
        const mat = (obj as ImmediateRenderObject).material as MeshBasicMaterial
        if (mat.map) {
            mat.map.dispose()
        }
        mat.map = await this.loadTexture(`${RenderConfig.config.baseUrl}/pano/${this.guid}/${obj.name}.jpg`);
