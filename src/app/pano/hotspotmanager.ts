...
export default class HotspotManager {
    ...
    onClick(){
        ...
        if(ls.length){
            const hp = currPano.links.find(x=>x.obj==ls[0].object)
            if(hp){
                if(hp.type==1)
    ...
    init(){
        ...
        air2 = new Sprite(airmat)
        air2.scale.set(0.6, 0.6, 0.6)

        air3 = new Sprite(airmat)
        air3.scale.set(0.6, 0.6, 0.6)

        air4 = new Sprite(airmat)
        air4.scale.set(0.6, 0.6, 0.6)

        air5 = new Sprite(airmat)
        air5.scale.set(0.6, 0.6, 0.6)
    ...
    hotspotRaycast(): Intersection[] {
        ...
        const intersects = raycaster.intersectObjects(currPano.links.filter(x=>x.obj).map(x=> x.obj as Object3D), true);
    ...
    findNearest(pos: Vector3): Object3D|null {
        ...
        for (const hp of currPano.links.filter(x=>x.obj && x.type==HotspotType.ground).map(x=>x.obj as Object3D)) {
    ...
    load(pano: Pano){
        if(currPano){
            for (const item of currPano.links) {
                if(item.obj){
        ...
        for (const item of pano.links) {
            const pos = item.pos.clone().sub(pano.pos)
            if(item.type==HotspotType.ground){
                item.obj = ground.clone()
            ...
            if(item.type==HotspotType.air){
                item.obj = air.clone()
                pos.multiplyScalar(10 / pos.length())
            }

            if(item.type==HotspotType.info){
                item.obj = info.clone()
                pos.multiplyScalar(10 / pos.length())
            }

            if(item.type==HotspotType.air2){
                item.obj = air2.clone()
                pos.multiplyScalar(10 / pos.length())
            }
            if(item.type==HotspotType.air3){
                item.obj = air3.clone()
                pos.multiplyScalar(10 / pos.length())
            }
            if(item.type==HotspotType.air4){
                item.obj = air4.clone()
                pos.multiplyScalar(10 / pos.length())
            }
            if(item.type==HotspotType.air5){
                item.obj = air5.clone()
                pos.multiplyScalar(10 / pos.length())
            }
            if(item.obj){
                item.obj.position.set(-pos.y, pos.z, pos.x)
                currPano.obj.add(item.obj)
            }
        }
    }
    update(){
        ...
        if(clickable.object!= clickabletarget){
            ...
            if(clickable.object){
                const hp = currPano.links.find(x=>x.obj==clickable.object)
                if(!hp || !hp.name){
    ...
    getinfopos():Array<Hotspot>{
        ...
        for (const item of currPano.links.filter(x=>x.type!=HotspotType.ground)) {
